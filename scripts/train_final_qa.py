...
def train(args, train_dataset, model, tokenizer, dev_dataset=None, dev_examples=None, dev_features=None):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=4*args.n_gpu)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    wandb.log({"lr":scheduler.get_last_lr()[0], "global_step": global_step,
                               "loss": (tr_loss - logging_loss) / args.logging_steps})
                    logging_loss = tr_loss
            
            if (args.eval_steps != -1) and ((step + 1) % args.eval_steps == 0):
                if args.local_rank in [-1, 0]:
                    if args.local_rank == -1 and args.do_eval and args.evaluate_during_training:
                        ...
                        logger.info(f"***** Eval Results (running) : {step}*****")
                        for key, value in results.items():
                            wandb.log({f"eval_{key}":value, "global_step":global_step})
                            logger.info("  %s = %s", key, round(value, 4))
                        # if results['F1'] > best_f1:
                        #     best_f1 = results['F1']
                        if args.do_eval_global == True:
                            assert args.do_eval == True, "do_eval must be set to true to perform do_eval_global."
                            with open(args.dev_global_path, 'r') as f:
                                dev_global = json.load(f)
                            global_questions = set([x["question_id"] for x in dev_global])
                            global_examples = [x for x in dev_examples if x.qas_id in global_questions]
                            global_nbests = dict()
                            for qas_id, v in nbests.items():
                                if qas_id in global_questions:
                                    global_nbests[qas_id] = v 

                            assert (len(global_examples) == len(global_questions)) and (len(global_nbests.keys()) == len(global_questions))

                            global_results = get_raw_scores_finer(global_examples, global_nbests)
                            logger.info(f"***** Eval results (Global / runnig) : {step} *****")
                            for key, value in global_results.items():
                                wandb.log({f"eval_global_{key}":value, "global_step":global_step})
                                logger.info("  %s = %s", key, round(value, 4))
        ...
        # Evaluate and save model checkpoint (after 1 epoch)
        if args.local_rank in [-1, 0]:
            if args.local_rank == -1 and args.do_eval and args.evaluate_during_training:
                predictions, nbests = evaluate_simplified(dev_dataset, dev_examples, dev_features, args, model, tokenizer, prefix='dev')
                # results = get_raw_scores(dev_examples, nbests)
                ...
                if args.do_eval_global == True:
                    ...
                    global_results = get_raw_scores_finer(global_examples, global_nbests)
                    logger.info("***** Eval results (Global / runnig) *****")
                    for key, value in global_results.items():
                        wandb.log({f"eval_global_{key}":value, "global_step":global_step})
                        logger.info("  %s = %s", key, round(value, 4))
...
def evaluate_simplified(dataset, examples, features, args, model, tokenizer, prefix=""):
    ...
    eval_dataloader = DataLoader(dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=args.n_gpu * 4)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.do_eval_global:
            ...
            assert (len(global_examples) == len(global_questions)) and (len(global_nbests.keys()) == len(global_questions))
            global_results = get_raw_scores_finer(global_examples, global_nbests)
            logger.info("***** Eval results (Global) *****")
            logger.info(f"Evalate on {len(global_examples)} examples")
            for key in global_results.keys():
                logger.info("  Eval %s = %s", key, str(global_results[key]))
            logger.info("Eval Model From: {}".format(os.path.join(output_dir, 'pytorch_model.bin')))
            logger.info("***** Eval results (Global) *****")
