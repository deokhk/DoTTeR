...
def prepare_test_data(ziped_data, topk_tbs, max_passages=3, max_tokens=360, max_cell_tokens=36, max_header_cell_tokens=20):
    # load data
    data, retrieval_output = ziped_data[0], ziped_data[1]
...
if __name__ == '__main__':
    ...
    OTTER_res_file = "/home/deokhk/research/OTT-QA-based-research/FR_with_rank/paper_writing/case_study/OTTER_dev_output_k100_table_corpus_blink.json"
    RATRO_res_file = "/home/deokhk/research/OTT-QA-based-research/FR_with_rank/paper_writing/case_study/RATRO_dev_output_k100_table_corpus_blink.json"
    n_threads = 24
    if args.split in ['train', 'dev']:
        ...
        with open(f'{basic_dir}/data_ottqa/{args.split}.traced.json', 'r') as f:
            data = json.load(f)
        OTTER_retrieval_outputs = read_jsonl(OTTER_res_file)
        RATRO_retrieval_outputs = read_jsonl(RATRO_res_file)
        ...
        dev_global_p = os.path.join(ottqa_path, "dev_global.json")
        with open(dev_global_p, 'r') as f:
            dev_global = json.load(f)
        
        dev_global_qids = [item['question_id'] for item in dev_global]
        running_function = prepare_training_data_random_nega


        # First get the result of OTTER 
        zip_data = list(zip(data, OTTER_retrieval_outputs))
        ...
        global_results_otter = []
        for res in all_results:
            qid = res["qa_data"]["question_id"]
            if qid in dev_global_qids:
                global_results_otter.append(res)

        global_results_otter_nohit = [x for x in global_results_otter if x['hit'] == 0]
        otter_nohit_qids = [x['qa_data']['question_id'] for x in global_results_otter_nohit]

        # Now get the result of RATRO 

        zip_data = list(zip(data, RATRO_retrieval_outputs))
        with Pool(n_threads) as p:
            func_ = partial(running_function, table_path=table_path, request_path=request_path,
                            topk_tbs=1,
                            max_passages=args.max_passages, max_tokens=args.max_tokens,
        ...
        global_results_ratro = []
        for res in all_results:
            qid = res["qa_data"]["question_id"]
            if qid in dev_global_qids:
                global_results_ratro.append(res)

        ratro_hit_otter_nohit = []
        ratro_hit_otter_nohit_qids = []
        for res in global_results_ratro:
            ishit = res['hit']
            qid = res["qa_data"]["question_id"]
            if (ishit == 1) and (qid in otter_nohit_qids):
                ratro_hit_otter_nohit.append(res)
                ratro_hit_otter_nohit_qids.append(qid)

        # Save it as a pair, per each question id 


        def filter_res(res):
            retrieved_block = res["qa_data"]["retrieved_tbs"][0]
            return retrieved_block
        case_study_res = []
        for otter_res, ratro_res in zip(global_results_otter, global_results_ratro):
            otter_qid = otter_res['qa_data']['question_id']
            ratro_qid = ratro_res['qa_data']['question_id']
            assert otter_qid == ratro_qid

            if otter_qid in ratro_hit_otter_nohit_qids:
                question = otter_res['qa_data']['question']
                answer = otter_res['qa_data']['answer-text']
                table_id = otter_res['qa_data']['table_id']
                gt_blocks = otter_res['qa_data']['positive_table_blocks'][0]
                case_study_res.append({'question_metadata': {
                    'qid': otter_qid, 'question': question, 'answer': answer, 'table_id': table_id, 'gt_blocks': gt_blocks}
                    ,'otter': filter_res(otter_res), 'ratro': filter_res(ratro_res)})


        print(f"{len(ratro_hit_otter_nohit)} questions are hit by RATRO but not OTTER")

        save_path = f'{basic_dir}/case_study_result.json'
        with open(save_path, 'w') as f:
            json.dump(case_study_res, f, indent=4)
