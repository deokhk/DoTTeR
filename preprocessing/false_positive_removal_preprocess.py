...
def prepare_train(zipped_data, table_path, request_path):
    # load data
    ...
    # If there are multiple positive rows, we ignore this instance
    if len(all_positive_row_index) > 1:
        return {'classifier_data': []}


    assert len(all_positive_row_index) == 1, "Zero or multiple positive rows found."
    ...
    return {'classifier_data': processed_data}



def prepare_dev(zipped_data, table_path, request_path):
    # load data
    # compared to prepare_train, as a negative sample we use every table in the dataset 
    ...
    for i, row in enumerate(contents):
        ...
        if i in all_positive_row_index:
            ...
            # Add random negative validation sample

            example['rd_neg_table'] = raw_tables[neg_i]
            example['rd_neg_passages'] = passages[neg_i]
            example['rd_neg_passages_id'] = passages_index[neg_i]
            example['rd_neg_row_id'] = neg_i

            # Add BM25 negative validation sample 
            corpus = [' '.join(row)+' '.join(p) for row, p in zip(contents, passages)]
            tokenized_corpus = [doc.split(" ") for doc in corpus]
            bm25 = BM25Okapi(tokenized_corpus)
            query = data['question'].split(' ')
            doc_scores = bm25.get_scores(query)
            index = np.array([0 if i in all_positive_row_index else 1 for i in range(len(doc_scores))])
            bm25_neg_i = (doc_scores*index).argmax()
            example['bm25_neg_table'] = raw_tables[bm25_neg_i]
            example['bm25_neg_passages'] = passages[bm25_neg_i]
            example['bm25_neg_passages_id'] = passages_index[bm25_neg_i]
            example['bm25_neg_row_id'] = bm25_neg_i
...
def get_filename(args):
    ...
    if args.aug_blink:
        prefix += '_blink'
    prefix += '_false_positive_removal'
    prefix += '_{}'.format(args.prefix) if args.prefix else ''

    save_path = '{}/preprocessed_data/false_positive_removal/{}_{}.pkl'.format(basic_dir, args.split, prefix)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nega', default='intable_random', type=str, choices=['intable_random', 'intable_contra', 'intable_bm25'], help="Negative sample selection scheme. For validation, we use intable_random along with intable_bm25.")
    ...
    if args.split in ['train', 'dev']:
        ...
        if not os.path.exists(save_path) or args.reprocess:
            ...
            run_func = prepare_train if args.split == "train" else prepare_dev

            
            with Pool(n_threads) as p:
                func_ = partial(run_func, table_path=table_path, request_path=request_path,)
                all_results = list(tqdm(p.imap(func_, zipped_data, chunksize=16), total=len(data), desc="convert examples to trainable data",))
                results = [res['classifier_data'] for res in all_results if res['classifier_data'] != []]
            
            logger.info(f"Out of {len(data)} examples, {len(results)} examples have only one positive row")
